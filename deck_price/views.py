...
def index(request):
    ...
    if data:
        ...
        url = 'http://%s%s' % (request.META['HTTP_HOST'], HttpRequest.get_full_path(request))
        api_user = settings.BITLY_API_USER
        api_key = settings.BITLY_API_KEY
        b = bitlyapi.BitLy(api_user, api_key)
        res = b.shorten(longUrl='%s' % url)

        return render(request, 'list.html', {'results': results, 'total': deck_total, 'short': res['url']})
